...
/** The class for the circle drawn
...
class DynamicCircle extends Circle {
	 ...
	 * @param endPoint The second point to calculate the radius
	 */
	public DynamicCircle(Point2D origin, Point2D endPoint) {
		super(origin.getX(), origin.getY(), origin.distance(endPoint));
		setOnMousePressed(new CirclePressedEventHandler());
		setOnMouseDragged(new CircleDraggedEventHandler());
		setOnScroll(new CircleScrolledEventHandler());
		...
		setOnContextMenuRequested(new CircleContextEventHandler(shapeMenu));
		MenuItem rotateItem = new MenuItem("Rotate");
		rotateItem.setOnAction(new CircleRotateEventHandler());
	...
	private class CircleScrolledEventHandler implements EventHandler<ScrollEvent> {
		...
		public void handle(ScrollEvent se) {
			...
			s.setPivotX(getCenterX());
			s.setPivotY(getCenterY());
			getTransforms().add(s);
		}
	}
	/** A class that deals with showing the context menu on the circle */
	private class CircleContextEventHandler implements EventHandler<ContextMenuEvent> {
		/** Variable that holds the context menu */
		ContextMenu shapeMenu;
		/** Main constructor
		 * @param shapeMenu The context menu to show
		 */
		CircleContextEventHandler(ContextMenu shapeMenu) {
		...
		public void handle(ContextMenuEvent cme) {
			...
			shapeMenu.show(DynamicCircle.this, cme.getScreenX(), cme.getScreenY());
		}
	}
	/** A class that deals with rotating the circle */
	private class CircleRotateEventHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent ae) {
			...
			try {
				...
				Rotate r = new Rotate();
				r.setAngle(angle);
				r.setPivotX(getCenterX());
				r.setPivotY(getCenterY());
				getTransforms().add(r);
